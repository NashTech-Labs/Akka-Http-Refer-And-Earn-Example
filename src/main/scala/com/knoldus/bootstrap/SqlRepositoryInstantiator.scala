...
import com.knoldus.db.{UserDataRepositorySqlImpl, UserMappedReferralCodeRepositorySqlImpl}
...
class SqlRepositoryInstantiator(dbConfig: Config)(implicit ec: ExecutionContext) extends RepositoryInstantiator {
  ...
  override val userDataRepository: UserDataRepository = new UserDataRepositorySqlImpl(db)
  override val userReferralCodeMappingRepository: UserReferralCodeMappingRepository = new UserMappedReferralCodeRepositorySqlImpl(db)
